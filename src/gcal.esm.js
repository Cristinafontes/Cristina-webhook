...
function normalizePtBrText(input) {
  return String(input || "")
    .toLowerCase()
    // remover acentos comuns para evitar problemas de fronteira de palavra
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
    // tratar "às" de forma robusta -> espaço
    .replace(/as/gi, " ")
    // remover palavras que atrapalham
    .replace(/horario/gi, "")
    .replace(/horário/gi, "")
    .replace(/,/g, " ")
    .replace(/\s{2,}/g, " ")
...
function buildIsoRange(d, m, y, HH, MM) {
  ...
  const offsetMinutes = /sao_paulo|são_paulo|sao-paulo/i.test(TZ) ? 180 : 0;
 ...
 *  - dd/mm[/aa] H[H]am|pm   (sem espaço, ex.: "15/10 10am")
 */
export function parseBRDateTime(text) {
  ...
  // 4) dd/mm[/aa] H[H]am|pm (sem espaço)
...
// Garantia de export explícito (caso o bundler exija)
export default { cancelEventFromMessage, parseBRDateTime };
