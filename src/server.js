...
async function handleInbound(req, res) {
  try {
    ...
    // Robust payload normalization (Gupshup may send JSON or urlencoded with 'payload' string)
    let raw = req.body;
    let payload;
    try {
      if (raw && typeof raw === "object" && typeof raw.payload === "string") {
        try { payload = JSON.parse(raw.payload); } catch { payload = raw; }
