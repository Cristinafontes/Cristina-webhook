...
// Captura o raw-body para validar assinatura se necessário
app.use(async (req, res, next) => {
  try {
    if (req.method === "POST") {
      req.rawBody = await getRawBody(req, { encoding: "utf-8" });
      try {
        req.body = JSON.parse(req.rawBody);
      } catch {
        // pode ser form-encoded ou texto simples; deixa vazio
        req.body = {};
      }
    }
  } catch (err) {
...
async function handleInbound(req, res) {
  try {
    ...
    // Tenta extrair data/hora da mensagem mais recente
    const { found, startISO, endISO } = parseCandidateDateTime(text, "America/Sao_Paulo");

    // Se encontramos uma data/horário, criamos o evento enriquecido
    if (found) {
    ...
    return res.status(200).json({ ok: true }); // evita reentrega
  }
}
